...
internal struct ShellCommandResult {
    ...
    func validate() -> Result<ShellCommandResult, ShellCommandFailure> {
        switch (self.code, self.terminationReason) {
        case (0, .exit): .success(self)
        case (_, .uncaughtSignal): .failure(ShellCommandFailure(exitCode: self.code, uncaughtSignal: true))
        default: .failure(ShellCommandFailure(exitCode: self.code, uncaughtSignal: false))
        }
    }
}
